...
!(function (a, b) {
  //lzsheng-2019-3-26 (fix 用户调整手机字体大小，对浏览器默认字体的影响，所以要除以跟标准浏览器16px的缩放系数)
  var user_webset_font = 16; //默认为16
  if (a.document.documentElement.currentStyle) {
    user_webset_font = a.document.documentElement.currentStyle["fontSize"];
  } else {
    user_webset_font = getComputedStyle(a.document.documentElement, false)[
      "fontSize"
    ];
  }
  var userFontSizeScale = parseFloat(user_webset_font) / 16;
  console.log("userFontSizeScale", userFontSizeScale);

  function c() {
    var b = f.getBoundingClientRect().width;
    // console.log('getBoundingClientRect().width',b)
    b / i > 540 && (b = 540 * i);
    // var c = b / 10;
    var c = b / 10 / userFontSizeScale; //lzsheng-2019-3-26 (fix 用户调整手机字体大小，对浏览器默认字体的影响，所以要除以跟标准浏览器16px的缩放系数)
